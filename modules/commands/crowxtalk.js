...
module.exports.run = async function({ api, event, args }) {
    ...
    if (action === "on") {
        simSimiEnabled = true;
        return api.sendMessage("Crowx auto-reply is now ON.", threadID, messageID);
    } else if (action === "off") {
        simSimiEnabled = false;
        return api.sendMessage("Crowx auto-reply is now OFF.", threadID, messageID);
    } else {
        if (!simSimiEnabled) {
            return api.sendMessage("Crowx auto-reply is currently OFF. Use 'autotalk on' to enable.", threadID, messageID);
        }

        api.sendMessage("Invalid command. You can only use 'crowxtalk on' or 'crowxtalk off'.", threadID, messageID);
    }
};
